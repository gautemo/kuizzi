...
const colors = [
  '#C0392B', '#E74C3C', '#f78fa6', '#9B59B6', '#8E44AD', '#2980B9', '#3498DB', '#1ABC9C', '#16A085', '#27AE60',
...
<template>
  ...
  <section v-else-if="step === 1">
    <p>Chose your icon</p>
    <div class="select">
      <span class="icon" v-for="i in icons" :key="i" @click="icon = i" :class="{selected: icon === i}" tabindex="0" @keyup.enter="icon = i; nextAfterIcon?.focus()">{{i}}</span>
    </div>
    <div class="buttons">
  ...
  <section v-else-if="step === 2">
    ...
    <div class="select colors">
      <span class="color" v-for="c in colors" :key="c" @click="color = c" :class="{selected: color === c}" :style="{background: c}" @keyup.enter="color = c; nextAfterColor?.focus()" tabindex="0"></span>
    </div>
    <div class="buttons">
      <button @click="step--" class="fancy">Back</button>
